...
export default function VendorOrdersPage() {
    ...
    const filtered = orders.filter(o => {
        ...
        const matchSearch = !ms || o.reference.toLowerCase().includes(ms) || o.recipient_name.toLowerCase().includes(ms) || o.delivery_address.toLowerCase().includes(ms);
        ...
        <div style={{ minHeight: '100vh', background: '#f8f9ff', fontFamily: "'Segoe UI', sans-serif" }}>
            ...
            <div style={{ maxWidth: '1080px', margin: '2rem auto', padding: '0 1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                    <div>
                        <h1 style={{ color: '#1a2456', fontSize: '1.4rem', fontWeight: 800, margin: 0 }}>My Orders</h1>
                        <p style={{ color: '#6b7280', fontSize: '0.88rem', marginTop: '0.25rem' }}>{orders.length} total orders</p>
                    </div>
                    <Link href="/vendor/orders/new" style={{ background: '#1a2456', color: 'white', padding: '0.65rem 1.25rem', borderRadius: '9px', textDecoration: 'none', fontWeight: 600, fontSize: '0.875rem', display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
                    {[
                        ...
                        <div key={s.label} style={{ background: 'white', borderRadius: '12px', padding: '1rem 1.1rem', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', border: '1px solid #efefef' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by reference, recipient or address..." style={{ flex: 1, minWidth: '220px', padding: '0.65rem 1rem', border: '1.5px solid #e2e8f0', borderRadius: '8px', fontSize: '0.875rem', outline: 'none', fontFamily: 'inherit', background: 'white' }} />
                    <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: '0.65rem 1rem', border: '1.5px solid #e2e8f0', borderRadius: '8px', fontSize: '0.875rem', outline: 'none', fontFamily: 'inherit', background: 'white', color: '#1a2456' }}>
                ...
                {loading ? (
                    ...
                    <div style={{ background: 'white', borderRadius: '14px', padding: '3rem', textAlign: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                        <p style={{ color: '#9ca3af', marginBottom: '1.25rem', fontSize: '0.9rem' }}>
                            {search || filterStatus !== 'All' ? 'No orders match your search' : 'No orders yet'}
                        </p>
                        {!search && filterStatus === 'All' && (
                            <Link href="/vendor/orders/new" style={{ background: '#1a2456', color: 'white', padding: '0.7rem 1.4rem', borderRadius: '8px', textDecoration: 'none', fontWeight: 600, fontSize: '0.875rem' }}>Create First Order</Link>
                        )}
                    </div>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                        {filtered.map(order => (
                            <div key={order.id} onClick={() => setSelected(order)} style={{ background: 'white', borderRadius: '14px', padding: '1.25rem 1.5rem', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', border: '1px solid #efefef', cursor: 'pointer' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                                    <div>
                                        <div style={{ fontWeight: 800, color: '#1a2456', fontSize: '0.95rem', fontFamily: 'monospace' }}>{order.reference}</div>
                                        <div style={{ color: '#6b7280', fontSize: '0.8rem', marginTop: '0.2rem' }}>{new Date(order.created_at).toLocaleDateString('en-GH', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                                    </div>
                                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                        <span style={{ background: STATUS_COLORS[order.status]?.bg || '#f3f4f6', color: STATUS_COLORS[order.status]?.color || '#374151', padding: '0.28rem 0.85rem', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 700 }}>{order.status}</span>
                                ...
                                <div style={{ marginBottom: '1rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.4rem' }}>
                                        {STATUS_STEPS.map((s, i) => (
                                            <div key={s} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                                                <div style={{ width: '20px', height: '20px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '0.25rem', background: i <= currentStep(order.status) ? (STATUS_COLORS[order.status]?.color || '#1a2456') : '#e5e7eb', color: 'white', fontSize: '0.65rem', fontWeight: 700 }}>
                                                    {i < currentStep(order.status) ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg> : i + 1}
                                                </div>
                                    ...
                                    <div style={{ position: 'relative', height: '3px', background: '#e5e7eb', borderRadius: '2px', margin: '0 10px' }}>
                                        <div style={{ position: 'absolute', left: 0, top: 0, height: '100%', borderRadius: '2px', background: STATUS_COLORS[order.status]?.color || '#1a2456', width: `${Math.max(0, (currentStep(order.status) / (STATUS_STEPS.length - 1)) * 100)}%` }} />
                                    </div>
                                </div>
            ...
            {/* Order detail modal */}
            {selected && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '1rem' }} onClick={() => setSelected(null)}>
                    <div style={{ background: 'white', borderRadius: '16px', padding: '2rem', maxWidth: '500px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' as const }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                            <div>
                                ...
                                <div style={{ color: '#9ca3af', fontSize: '0.78rem', marginTop: '0.2rem' }}>{new Date(selected.created_at).toLocaleString('en-GH')}</div>
                            </div>
                        ...
                        <div style={{ background: '#f8f9ff', borderRadius: '12px', padding: '1.25rem', marginBottom: '1.25rem' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                {STATUS_STEPS.map((s, i) => (
                                    <div key={s} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                                        <div style={{ width: '26px', height: '26px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '0.35rem', background: i <= currentStep(selected.status) ? (STATUS_COLORS[selected.status]?.color || '#1a2456') : '#e5e7eb', color: 'white', fontSize: '0.7rem', fontWeight: 700 }}>
                                            {i < currentStep(selected.status) ? <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg> : i + 1}
                                        </div>
