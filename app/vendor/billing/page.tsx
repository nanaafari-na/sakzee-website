...
export default function VendorBillingPage() {
    ...
    function payInvoice(invoice: Invoice) {
        ...
        const handler = window.PaystackPop.setup({
            ...
            onClose: function () { setPaying(null); },
        });
        handler.openIframe();
    }
    ...
    function formatMonth(month: string) {
        ...
        return new Date(Number(year), Number(m) - 1).toLocaleDateString('en-GH', { month: 'long', year: 'numeric' });
    }
        ...
        <div style={{ minHeight: '100vh', background: '#f8f9ff', fontFamily: "'Segoe UI', sans-serif" }}>
            ...
            <div style={{ maxWidth: '860px', margin: '2rem auto', padding: '0 1rem' }}>
                <div style={{ marginBottom: '1.75rem' }}>
                    <h1 style={{ color: '#1a2456', fontSize: '1.4rem', fontWeight: 800, margin: 0 }}>Storage Billing</h1>
                    <p style={{ color: '#6b7280', fontSize: '0.88rem', marginTop: '0.25rem' }}>Monthly invoices — billed daily based on shelf and pallet spaces used</p>
                </div>

                {/* Summary cards */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                    {[
                        { label: 'Outstanding Balance', value: `GHS ${totalUnpaid.toFixed(2)}`, color: totalUnpaid > 0 ? '#dc2626' : '#15803d', bg: totalUnpaid > 0 ? '#fef2f2' : '#f0fdf4', border: totalUnpaid > 0 ? '#fecaca' : '#bbf7d0' },
                        ...
                        { label: 'Unpaid Invoices', value: String(invoices.filter(i => i.status === 'unpaid').length), color: invoices.filter(i => i.status === 'unpaid').length > 0 ? '#dc2626' : '#15803d', bg: '#f8f9ff', border: '#e2e8f0' },
                    ].map(s => (
                ...
                {error && <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '10px', padding: '0.75rem 1rem', color: '#dc2626', fontSize: '0.875rem', marginBottom: '1.25rem' }}>{error}</div>}

                {loading ? (
                    <div style={{ background: 'white', borderRadius: '14px', padding: '3rem', textAlign: 'center', color: '#9ca3af' }}>Loading invoices...</div>
                ) : invoices.length === 0 ? (
                    <div style={{ background: 'white', borderRadius: '14px', padding: '3rem', textAlign: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                        <p style={{ color: '#9ca3af', fontSize: '0.9rem' }}>No invoices yet — invoices are generated once you store inventory</p>
                    </div>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {invoices.map(inv => (
                            <div key={inv.id} style={{ background: 'white', borderRadius: '14px', padding: '1.5rem', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', border: `1px solid ${inv.status === 'unpaid' ? '#fecaca' : '#e5e7eb'}` }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.25rem' }}>
                                    ...
                                    <span style={{ background: inv.status === 'paid' ? '#f0fdf4' : '#fef2f2', color: inv.status === 'paid' ? '#15803d' : '#dc2626', padding: '0.35rem 1rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 700 }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
                                    <div style={{ background: '#f0f3ff', borderRadius: '10px', padding: '0.9rem 1rem' }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            ...
                                            <div style={{ fontWeight: 800, color: '#1a2456' }}>GHS {(inv.shelf_days * inv.shelf_rate).toFixed(2)}</div>
                                        </div>
                                    </div>
                                    <div style={{ background: '#fff3e8', borderRadius: '10px', padding: '0.9rem 1rem' }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            ...
                                            <div style={{ fontWeight: 800, color: '#1a2456' }}>GHS {(inv.pallet_days * inv.pallet_rate).toFixed(2)}</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f3f4f6', paddingTop: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
                                    ...
                                    {inv.status === 'unpaid' && (
                                        <button onClick={() => payInvoice(inv)} disabled={paying === inv.id} style={{ background: paying === inv.id ? '#ccc' : '#f97316', color: 'white', border: 'none', padding: '0.8rem 1.75rem', borderRadius: '9px', fontWeight: 700, fontSize: '0.95rem', cursor: paying === inv.id ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
