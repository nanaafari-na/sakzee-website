...
export default function EditProductPage({ params }: { params: { id: string } }) {
    ...
    async function loadProduct(token: string) {
        try {
            ...
            setForm({ name: data.name || '', sku: data.sku || '', category: data.category || '', low_stock_threshold: String(data.low_stock_threshold || 10), space_type: data.space_type || 'shelf' });
    ...
    async function handleSave() {
        ...
        try {
            const res = await fetch(`/api/vendor/products?id=${params.id}`, {
                ...
                body: JSON.stringify({ name: form.name, sku: form.sku, category: form.category, low_stock_threshold: Number(form.low_stock_threshold), space_type: form.space_type }),
            });
            const data = await res.json();
    ...
    if (loading) return (
        <div style={{ minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", background: '#f8f9ff' }}>
            <VendorNav />
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '4rem', color: '#9ca3af' }}>Loading product...</div>
        </div>
    );
        ...
        <div style={{ minHeight: '100vh', background: '#f8f9ff', fontFamily: "'Segoe UI', sans-serif" }}>
            ...
            <div style={{ maxWidth: '540px', margin: '3rem auto', padding: '0 1rem' }}>
                ...
                <div style={{ background: 'white', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 24px rgba(0,0,0,0.07)' }}>
                    ...
                    {success && <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '8px', padding: '0.75rem 1rem', color: '#15803d', fontSize: '0.875rem', marginBottom: '1.25rem' }}>✓ Product updated! Redirecting...</div>}

                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
                        <div><label style={lbl}>Product Name *</label><input style={inp} name="name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Men's White T-Shirt" /></div>
                        <div><label style={lbl}>SKU (optional)</label><input style={inp} name="sku" value={form.sku} onChange={e => setForm({ ...form, sku: e.target.value })} placeholder="e.g. SHIRT-WHT-M" /></div>
                        <div><label style={lbl}>Category (optional)</label><input style={inp} name="category" value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} placeholder="e.g. Fashion, Electronics, Food" /></div>
                        <div><label style={lbl}>Low Stock Alert At</label><input style={inp} name="low_stock_threshold" type="number" min="0" value={form.low_stock_threshold} onChange={e => setForm({ ...form, low_stock_threshold: e.target.value })} placeholder="10" /></div>
                        <div>
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem' }}>
                            <button onClick={handleSave} disabled={saving || success} style={{ flex: 1, background: saving || success ? '#ccc' : '#1a2456', color: 'white', border: 'none', padding: '0.95rem', borderRadius: '10px', fontSize: '1rem', fontWeight: 700, cursor: saving || success ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
