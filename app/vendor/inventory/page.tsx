...
type Product = { id: string; name: string; sku: string; category: string; quantity: number; space_type: string; low_stock_threshold: number; checkin_status: string; checked_in_quantity: number; checked_in_at: string | null; created_at: string; };
    ...
    return (
      ...
      `}</style>
            ...
            <nav style={{ background: '#1a2456', padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div className="inv-desktop-nav" style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
            ...
            <div style={{ maxWidth: '1080px', margin: '2rem auto', padding: '0 1rem' }}>
                <div className="inv-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '1rem' }}>
                    ...
                    <Link href="/vendor/inventory/add" style={{ background: '#1a2456', color: 'white', padding: '0.65rem 1.25rem', borderRadius: '9px', textDecoration: 'none', fontWeight: 600, fontSize: '0.875rem', display: 'inline-flex', alignItems: 'center', gap: '0.4rem', whiteSpace: 'nowrap' }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>Add Product
                    </Link>
                </div>
                                    ...
                                    {filtered.map((p, i) => {
                                        ...
                                        return (
                                            <tr key={p.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                                                <td style={{ padding: '0.9rem 1rem' }}><div style={{ fontWeight: 600, color: '#1a2456' }}>{p.name}</div>{p.category && <div style={{ color: '#9ca3af', fontSize: '0.75rem' }}>{p.category}</div>}</td>
                                                <td style={{ padding: '0.9rem 1rem', color: '#6b7280', fontFamily: 'monospace', fontSize: '0.8rem' }}>{p.sku || '—'}</td>
                                                <td style={{ padding: '0.9rem 1rem' }}><span style={{ background: p.space_type === 'pallet' ? '#fff3e8' : '#f0f3ff', color: p.space_type === 'pallet' ? '#c2410c' : '#1d4ed8', padding: '0.2rem 0.65rem', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600 }}>{p.space_type === 'pallet' ? 'Pallet' : 'Shelf'}</span></td>
                                                <td style={{ padding: '0.9rem 1rem' }}><span style={{ background: stock.bg, color: stock.color, padding: '0.2rem 0.65rem', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 700 }}>{p.checkin_status === 'pending_checkin' ? 'Pending' : `${p.quantity} units`}</span></td>
                                                <td style={{ padding: '0.9rem 1rem' }}><span style={{ background: checkin.bg, color: checkin.color, padding: '0.2rem 0.65rem', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 700 }}>{checkin.label}</span></td>
                                                <td style={{ padding: '0.9rem 1rem', color: '#374151', fontSize: '0.82rem' }}>{p.checkin_status === 'checked_in' && p.checked_in_at ? new Date(p.checked_in_at).toLocaleDateString('en-GH', { day: 'numeric', month: 'short' }) : '—'}</td>
                                                <td style={{ padding: '0.9rem 1rem' }}>
                                                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                                                        ...
                                                        <button onClick={() => deleteProduct(p.id)} disabled={deleting === p.id} style={{ color: '#dc2626', background: 'none', border: '1px solid #fecaca', borderRadius: '6px', padding: '0.3rem 0.7rem', fontSize: '0.8rem', cursor: 'pointer', fontFamily: 'inherit' }}>{deleting === p.id ? '...' : 'Delete'}</button>
                                                    </div>
                                                </td>
                                            </tr>
                                        );
                ...
                <div className="inv-cards" style={{ display: 'none', flexDirection: 'column', gap: '0.85rem' }}>
                    {loading ? <div style={{ background: 'white', borderRadius: '12px', padding: '2rem', textAlign: 'center', color: '#9ca3af' }}>Loading...</div> :
                        filtered.length === 0 ? (
                            ...
                            return (
                                <div key={p.id} style={{ background: 'white', borderRadius: '12px', padding: '1.25rem', border: '1px solid #efefef', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
                                        ...
                                        <span style={{ background: stock.bg, color: stock.color, padding: '0.22rem 0.7rem', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700 }}>{p.checkin_status === 'pending_checkin' ? 'Pending stock' : `${p.quantity} units`}</span>
                                    </div>
                                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                                        ...
                                        <button onClick={() => deleteProduct(p.id)} disabled={deleting === p.id} style={{ flex: 1, color: '#dc2626', background: 'none', border: '1px solid #fecaca', borderRadius: '8px', padding: '0.55rem', fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'inherit' }}>{deleting === p.id ? '...' : 'Delete'}</button>
                                    </div>
                                </div>
                            );
                        })}
